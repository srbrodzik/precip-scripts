    ...
    try:
        ...
        if options.platform in platforms:
            ...
            for ifile in remoteFiles:
                if(options.verbose):
                    print('checking remoteFile:',ifile)
        ...
        if (options.verbose):
            print("==>> unable to run cwbFTP.cwd(thisDayStr)", file=sys.stderr)
...
def getTimeAndType(file):
    ...
    if options.platform == 'LIGHTNING':
        ...
        if options.ftpSubDir == 'MOSAIC2D' or options.ftpSubDir == 'MOSAIC3D':
            ...
            timeStr = date+time+'00'
            type = 'nexrad'
        elif options.ftpSubDir == 'SPOKA':
            (type,datetime,junk,junk,junk) = file.split('.')
            (date,time) = datetime.split('_')
            timeStr = date+time
            type = 'cfradial'
        elif options.ftpSubDir == 'TEAMR':
            (date,time,junk,junk,junk) = file.split('_')
            timeStr = date+time
            type = 'uf'
        else:
        ...
        if options.fileSuffix == 'bfr':
            if '_all_' in file:
                (junk,junk,date,time,junk) = file.split('_')
            else:
                (junk,date,time,junk) = file.split('_')
            timeStr = date+time+'00'
        else:
        ...
        timeStr = date+time
        type = 'txt'
    elif options.platform == 'SAT':
        (datetime,junk,junk,junk,junk,junk) = file.split('.')
        (date,time) = datetime.split('_')
        date = date.replace('-','')
        timeStr = date+time+'00'
        type = 'unknown'
    elif options.platform == 'WINDPRO':
        (datetime,junk) = file.split('_')
        datetime = datetime.replace('w','')
        (year,month,day,hour,min) = datetime.split('-')
        timeStr = year+month+day+hour+min+'00'
        type = options.fileSuffix
...
def parseArgs():
    ...
    if (startTime.year > 1970 and endTime.year > 1970):
        ...
        archiveMode = False
    #print('End of parseArgs:')
    #print('   startTime   =',startTime)
    #print('   endTime     =',endTime)
    #print('   archiveMode =',archiveMode)
