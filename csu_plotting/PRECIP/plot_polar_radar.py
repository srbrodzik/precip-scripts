...
#Brenda Dolan, May 2022 bdolan@colostate.edu
#Updating the plotting for PRECIP 2022
#  - Changing names to be consistent with the NCAR field catalogue
#  - Changing the PPI 6 panel plot to include HID
...
if radar.scan_type == 'ppi':
    ...
    ####Make the lowest elevation angle plot.
    # regardless of map or not, going to only plot the first PPI sweep, at least in real time
    
    ppilow = radar.extract_sweeps([0])
    dzlow = ppilow.fields[cfg.dz_name]['data']
    drlow = ppilow.fields[cfg.zdr_name]['data']
    kdlow = ppilow.fields[cfg.kdp_name]['data']
    

    latlow = ppilow.gate_latitude['data']
    lonlow = ppilow.gate_longitude['data']
    hidlow = ppilow.fields['HID']['data']
    
    yh_cs, yh_cc, yh_bkgnd = shy.conv_strat_latlon(dzlow, latlow, lonlow, 40.0, method='SYH', a=8, b=64)
    ...
    if 'rainmethod' in ycfg.keys() and ycfg['rainmethod'] == 'csutropical':
        rr_array, rr_meth = csu_blended_rain_tropical.calc_blended_rain_tropical(dz=dzlow, zdr=drlow, kdp=kdlow, 
                    cs=cs_arr, fhc=hid)

    elif 'rainmethod' in ycfg.keys() and ycfg['rainmethod'] == 'csuhidro':
        rr_array, rr_meth = csu_blended_rain.csu_hidro_rain(dz=dzlow, zdr=drlow, kdp=kdlow, fhc=hidlow, band=ycfg['radar_band'],
        ...
        rr_array, rr_meth = csu_blended_rain.calc_blended_rain(dz=dzlow, zdr=drlow, kdp=kdlow, ice_flag=False, band=ycfg['radar_band'],
    ...
    ppilow = radtools.add_field_to_radar_object(rr_array, ppilow, field_name='RR', units='%', 
				   long_name='Rain Rate',
 ...
 #       display = pyart.graph.RadarDisplay(ppilow)
        if 'map_flag' in ycfg.keys() and ycfg['map_flag']:
            # this means we want to plot on a map
            display = pyart.graph.RadarMapDisplay(ppilow)
...
#        ns=0    
        ...
        display_func(cfg.dz_name, sweep=0, vmin=cfg.plot_params[cfg.dz_name]['lims'][0], vmax=cfg.plot_params[cfg.dz_name]['lims'][1], 
        ...
        display_func(vel_name, sweep=0, vmin=-1*vmax, vmax=vmax, cmap=ctables.Carbone11, 
             colorbar_label='Radial velocity (m/s)', mask_outside=False, **extra_args)
        display.set_limits(xlim=limx, ylim=limy)


        ax2 = fig.add_subplot(323)
        display_func(cfg.zdr_name, sweep=0,  vmin=cfg.plot_params[cfg.zdr_name]['lims'][0], vmax=cfg.plot_params[cfg.zdr_name]['lims'][1], 
        ...
        display_func(cfg.kdp_name, sweep=0, vmin=-1.0, vmax=6.0, cmap=cfg.plot_params[cfg.kdp_name]['cmap'],
        ...
        display_func('RR', sweep=0, vmin=0,vmax=225,cmap=rr_cmap,norm=rr_norm,
             colorbar_label='%s %s'%('Rain Rate', 
		    cfg.plot_params[cfg.rr_name]['units']), mask_outside=False, **extra_args)
        display.set_limits(xlim=limx, ylim=limy)
...
#         display.set_limits(xlim=limx, ylim=limy)
        ...
        display_func('HID', sweep=0, cmap=cfg.plot_params['HID']['cmap'], norm=cfg.plot_params['HID']['norm'], 
        ...
        hidcb = cfg.HID_barplot_colorbar(fig, location=[0.885, 0.026, 0.018, 0.30])
        ...
        ptools.save_figure(fig, '%s/raw_%s/%s'%(fig_path, radar.scan_type, date_string), 
                'research.Radar_SEAPOL.%s.%sm_6vars_05.png'%(dt_string,radar.scan_type), dpi=DPI)
...
for i, ns in enumerate(sweeps):
    ...
    #print 'i: {}, ns: {}'.format(i, ns)
        ...
        ax0 = fig.add_subplot(321)
        display_func(cfg.dz_name, sweep=i, vmin=cfg.plot_params[cfg.dz_name]['lims'][0], vmax=cfg.plot_params[cfg.dz_name]['lims'][1], 
		cmap=cfg.plot_params[cfg.dz_name]['cmap'], colorbar_label='Reflectivity (dBZ)', mask_outside=True, **extra_args)
        display.set_limits(xlim=limx, ylim=limy)


        ax1 = fig.add_subplot(322)
        display_func(vel_name, sweep=i, vmin=-1*vmax, vmax=vmax, cmap=ctables.Carbone11, 
             colorbar_label='Radial velocity (m/s)', mask_outside=False, **extra_args)
        display.set_limits(xlim=limx, ylim=limy)


        ax2 = fig.add_subplot(323)
        display_func(cfg.zdr_name, sweep=i,  vmin=cfg.plot_params[cfg.zdr_name]['lims'][0], vmax=cfg.plot_params[cfg.zdr_name]['lims'][1], 
	       cmap=cfg.plot_params[cfg.zdr_name]['cmap'], colorbar_label='%s %s'%(cfg.plot_params[cfg.zdr_name]['name'], 
	    cfg.plot_params[cfg.zdr_name]['units']), mask_outside=False, **extra_args)
        display.set_limits(xlim=limx, ylim=limy)
        ...
        ax3 = fig.add_subplot(324)
        display_func(cfg.rho_name, sweep=i, vmin=0.92, vmax=cfg.plot_params[cfg.rho_name]['lims'][1], 
        ...
        display_func(cfg.kdp_name, sweep=i, vmin=-1.0, vmax=6.0, cmap=cfg.plot_params[cfg.kdp_name]['cmap'],
		 colorbar_label='%s %s'%(cfg.plot_params[cfg.kdp_name]['name'], cfg.plot_params[cfg.kdp_name]['units']), 
                mask_outside=False, **extra_args)
        display.set_limits(xlim=limx, ylim=limy)
...
#         display.set_limits(xlim=limx, ylim=limy)
        ...
        ax5 = fig.add_subplot(326)
        display_func('HID', sweep=i, cmap=cfg.plot_params['HID']['cmap'], norm=cfg.plot_params['HID']['norm'], 
                colorbar_label='HID', mask_outside=False, colorbar_flag=True)
        display.set_limits(xlim=limx, ylim=limy)
        ...
        hidcb = cfg.HID_barplot_colorbar(fig, location=[0.865, 0.03, 0.010, 0.28])
        hidcb.ax.tick_params(labelsize=9)
	    ...
	    #angle_title = np.average(radar.get_azimuth(i))
        eltitle = ns
        plt.tight_layout()
...
#research.Radar_SEAPOL.<datetime>.ppim_6vars_05.ext
        print '#######################################i is %s'%(i)
        ptools.save_figure(fig, '%s/raw_%s/%s'%(fig_path, radar.scan_type, date_string), 
                'research.Radar_SEAPOL.%s.%sm_6vars_%s.png'%(dt_string,radar.scan_type,eltitle), dpi=DPI)
...
if True:
    ...
    #research.Radar_SEAPOL.<datetime>.ppim_dbz_vel_05.ext
    #plt.savefig('%s/figures/large_dbz_vel_%s/%s%s_%s.png'%(base_path, radar.scan_type, prefix, dt_string, radar.scan_type), dpi=largeDPI)
        ptools.save_figure(fig, '%s/large_dbz_vel_%s/%s'%(fig_path, radar.scan_type, date_string), 
                    'research.Radar_SEAPOL.%s.%sm_dbz_vel_05.png'%(dt_string, radar.scan_type), dpi=largeDPI)
...
for i, ns in enumerate(sweeps):
    try:
        ...
        aznz = int(np.round(radar.fixed_angle['data'][i],decimals=0))
        print 'RHI AZIMUTH IS %s'%(aznz)
...
#                                radar.scan_type, i), dpi=DPI)
	ptools.save_figure(fig, '%s/rhi_true_aspect/%s'%(fig_path, date_string), 
                    'research.Radar_SEAPOL.%s.rhim.6vars.%s.png'%(dt_string, aznz), dpi=DPI)
