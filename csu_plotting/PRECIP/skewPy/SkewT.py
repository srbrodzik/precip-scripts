...
# Copyright (c) 2013 Thomas Chubb 
    ...
    parcel = False, parcel_draw = False, **kwargs):
        ...
        if isinstance(title, str):
            ...
            try:
                self.skewxaxis.set_title("%s, %sZ, Lon: %.2f$^{\circ}$, Lat: %.2f$^{\circ}$"%(self.station, self.sounding_date, self.lon, self.lat))
            except Exception:
                self.skewxaxis.set_title(self.station)
    ...
    def cape(self):
        ...
        # if np.where(np.where(CAPE < 0)[0][1:] - np.where(CAPE < 0)[0][:-1] > 1)[0].size == 0:
        #     CIN = 0
        # else:
        #     LFC_i = np.where(np.where(CAPE < 0)[0][1:] - np.where(CAPE < 0)[0][:-1] > 1)[0][0]
        #     CIN = CAPE[:LFC_i].sum()

        # CIN = np.ma.masked_greater(CAPE, 0)
        # CIN = CIN[np.where(self.data['pres'] > 500)].sum()
        CAPE = np.ma.masked_less(CAPE, 0)
        self.fig.text(0.84, 0.60, 'CAPE: %d J/kg'%(CAPE.sum()))
        ...
        # 50 mb mixed layer q
        ml_levels = (self.data['pres'][0] - self.data['pres']) <= 50.0
        e_sat_ml = VaporPressure(self.data['temp'][ml_levels])
        w_sat_ml = (e_sat_ml*Rs_da) / (Rs_v*(self.data['pres'][ml_levels]*100. - e_sat_ml))
        q_sat_ml = w_sat_ml/(w_sat_ml+1)

        e_ml = VaporPressure(self.data['dwpt'][ml_levels])
        w_ml = (e_ml*Rs_da) / (Rs_v*(self.data['pres'][ml_levels]*100. - e_ml))
        q_ml = w_ml/(w_ml+1)
        q_ml_avg = np.average(q_ml)
        ...
        return CAPE.sum(), self.data['pres'][-1], self.sounding_date, Tw, RH_sfc, q_sfc, q_ml_avg
        pass
    ...
    def add_profile(self,bloc=0.5, pres_s = 1000.0, **kwargs):
        ...
        except KeyError: raise KeyError as "Pressure in hPa (PRES) is required!"
	...
	print 'plotting the temp and dewpoint'
    ...
    def make_skewt_axes(self, pmax = 1100., pmin = 50.):
        ...
        self.fig.text(0.85, 0.16, 'Ht: %d m'%(self.data['hght'][0]))

        self.fig.text(0.85, 0.14, 'T: %.1f %s'%(self.data['temp'][0], degC))
        self.fig.text(0.85, 0.12, 'T$_D$: %.1f %s'%(self.data['dwpt'][0], degC))
        ##self.fig.text(0.85, 0.10, 'T$_W$: %.1f %s'%(tw, degC))
        ...
        if self.fmt == 'EDT' or self.fmt == 'EC' or self.fmt == 'XML':
        ...
        self.hodoax.set_yticks(np.arange(0, 60, 10))
    ...
    #--------------------------------------------------------------------
        if self.fmt == 'UWYO': # READING IN STANDARD UNIVERSITY OF WYOMING FILES
            ...
            for rows in root.findall('Row'):
                ...
                if i == 1:
                    starttime = datetime.strptime(rows.attrib.get('DataSrvTime')[:-1], '%Y-%m-%dT%H:%M:%S.%f')
                curtime = datetime.strptime(rows.attrib.get('DataSrvTime')[:-1], '%Y-%m-%dT%H:%M:%S.%f')

                if i == 0:
                    dt.append(-9999)
                else:
                    dt.append((curtime-starttime).total_seconds())
                #print dt[-1]
                i = i + 1
            ...
            self.station = 'SPURS2'
            self.sounding_date = starttime
            self.lon = lon[0]
            self.lat = lat[0]

            self.data = {'hght': Hgt, 'pres': P, 'temp': T, 'dwpt': Td, 'sknt': ws, 'drct': wd, 
                    'lat': lat, 'lon': lon, 'rh': RH, 'time': dt}
