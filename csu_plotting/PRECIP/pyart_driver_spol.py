...
def process_pipeline(fname):
    ...
    (junk,a,junk,junk,junk) = fbase.split('.')
	...
	#check_qc = glob.glob('%s/*/%s*%s*%s'%(qc_data_path, qc_prefix, a, qc_ftype))
	    ...
	    plot_cmd = 'python -W ignore %s/plot_polar_spol.py --file=%s --config=%s'%(base_path, fname, cfg_base)	
...
if len(raw_files):
    ...
    file_name_strings = np.array( [gtools.parse_time_string(os.path.basename(_)) for _ in raw_files] )
    file_time_list = []
    for file in file_name_strings:
        (file_time,junk,junk,junk) = file.split('.')
        file_time_list.append(file_time)
    file_time_strings = np.array(file_time_list)
